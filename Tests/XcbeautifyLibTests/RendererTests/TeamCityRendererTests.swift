...
struct TeamCityRendererTests {
    ...
    @Test func clangError() {
        ...
        #expect(
            formatted == "##teamcity[message text=\'Build error\' errorDetails=\'|[x|] clang: error: linker command failed with exit code 1 (use -v to see invocation)\' status=\'ERROR\']\nBuild error"
        )
    }
    ...
    @Test func compileAssetCatalog() {
        ...
        #expect(formatted == "[LayeredArtworkLibrary_LayeredArtworkLibrary] Compile Asset Catalog AssetCatalog.xcassets")
    }
    ...
    @Test func createUniversalBinary() {
        ...
        #expect(formatted == "[BackyardBirdsUITarget] Create Universal Binary BackyardBirdsUI")
    }
    ...
    @Test func generateAssetSymbols() {
        ...
        #expect(formatted == "[BackyardBirdsUI] Generate Asset Symbols SomeAssets.xcassets")
    }
    ...
    @Test func ldError() {
        ...
        #expect(noColoredFormatted(inputLdLibraryError) == "##teamcity[message text=\'Build error\' errorDetails=\'|[x|] ld: library not found for -lPods-Yammer\' status=\'ERROR\']\nBuild error")

        let inputLdSymbolsError = "ld: symbol(s) not found for architecture x86_64"
        #expect(noColoredFormatted(inputLdSymbolsError) == "##teamcity[message text=\'Build error\' errorDetails=\'|[x|] ld: symbol(s) not found for architecture x86_64\' status=\'ERROR\']\nBuild error")
    }
    ...
    @Test func undefinedSymbolLocation() {
        ...
        #expect(formatted == "##teamcity[message text=\'Undefined symbol location|n|[!|]       MediaBrowser.ChatGalleryViewController.downloadImage() -> () in MediaBrowser(ChatGalleryViewController.o)\' status=\'WARNING\']\nUndefined symbol location")
    }
    ...
    @Test func validate() {
        ...
        #expect(formatted == "[Target] Validate some.app")
    }
    ...
    @Test func xcodebuildError() {
        ...
        #expect(formatted == "##teamcity[message text=\'Build error\' errorDetails=\'|[x|] xcodebuild: error: Existing file at -resultBundlePath \"/output/file.xcresult\"\' status=\'ERROR\']\nBuild error")
    }
    ...
    @Test func duplicateLocalizedStringKey() {
        ...
        #expect(formatted == "##teamcity[message text=\'Duplicated localized string key|n|[!|] Key \"duplicate\" used with multiple values. Value \"First\" kept. Value \"Second\" ignored.\' status=\'WARNING\']\nDuplicated localized string key")
    }

    @Test func commandProduceError() {
        let error = """
        ...
        #expect(actual == "##teamcity[message text=\'Build error\' errorDetails=\'|[x|] error: Multiple commands produce |\'/Users/admin/teamcity/work/8906de356cda3a27/build/fastlane/Build/Products/Debug-iphonesimulator/some.app/PrivacyInfo.xcprivacy|\'\' status=\'ERROR\']\nBuild error")
    }
    ...
    @Test func swiftTestingSuiteFailed() {
        ...
        #expect(formatted == "##teamcity[message text=\'Suite failed\' errorDetails=\'\"MyTestSuite\" failed after 8.456 seconds with 2 issue(s)\' status=\'ERROR\']\nSuite failed")
    }
    ...
    @Test func swiftTestingTestSkipped() {
        ...
        let formatted = noColoredFormatted(input)
        #expect(formatted == "##teamcity[message text=\'Test skipped|n\"myTest\"\' status=\'WARNING\']\nTest skipped")
    }

    @Test func swiftTestingTestSkippedReason() {
        let input = #"􀙟 Test "myTest" skipped: "Reason for skipping""#
        let formatted = noColoredFormatted(input)
        #expect(formatted == "##teamcity[message text=\'Test skipped|n\"myTest\" (Reason for skipping)\' status=\'WARNING\']\nTest skipped")
    }
    ...
    @Test func testSessionResults() {
        let formatted = noColoredFormatted("Test session results, code coverage, and logs:")
